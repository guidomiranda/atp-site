...
import { deleteProducto } from '../../../../utils/producto';
...
const Clients: React.FC<ClientsProps> = ({ client }) => {
	...
	const handleDeleteClient = async () => {
		const response = await deleteProducto(client.id);
		...
		>
			...
			>
				<Button
					...
					_focus={{ shadow: 'none' }}
					onClick={() => router.push(`/admin/promociones/productos/${client.id}`)}
				>
					<FiEdit />
				</Button>
				<Button
			...
			footer={
				<Box>
					<Button
						...
						fontWeight='medium'
						onClick={() => router.push('/promociones/productos')}
					>
						Crear producto
					</Button>
				</Box>
