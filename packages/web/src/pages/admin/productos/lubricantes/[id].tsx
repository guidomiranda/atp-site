...
import { getLubricante, updateLubricante } from '../../../../utils/lubricants';
import produce from 'immer';
import { FaPlus, FaTrash } from 'react-icons/fa';

export const getServerSideProps: GetServerSideProps = async context => {
	const lubricantes = await getLubricante(context.query.id as string);
	return { props: { lubricantes: lubricantes.data } };
};

const FiltroAdminEdit = ({ lubricantes }) => {
	const router = useRouter();

	const [lubricantesInfo, setLubricantesInfo] = useState<any>(lubricantes);

	const inputImgRef = useRef(null);
	const [imageExist, setImageExist] = useState(lubricantes.imagen);
	...
	const handleUpdateSuccessInfo = async () => {
		if (image) {
			...
			delete lubricantesInfoUpdated.id;
			const response = await updateLubricante(
				lubricantesInfo.id,
				lubricantesInfoUpdated
			);
				...
				<Box w={['100%']}>
					<Box>
						...
						<Box mb='20px'>
							...
							<Box>
								{lubricantesInfo.imagen && (
									<Image
										src={image || lubricantesInfo.imagen}
										// w='200px'
										h='200px'
										verticalAlign='top'
										alt='Dan Abramov'
